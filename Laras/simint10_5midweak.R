...
y2<-0.05*x2+0.05*x90+0.05*x2*x90+e2
y3<-0.05*x2*x90+e2

data2<-data.frame(cbind(y2,x))
data3<-data.frame(cbind(y3,x))

write.table(data2, paste("DATA10_5intweakfullmid.",i, sep=""), row.names=FALSE, quote=FALSE)
write.table(data3, paste("DATA10_5intweakredmid.",i, sep=""), row.names=FALSE, quote=FALSE)
}
